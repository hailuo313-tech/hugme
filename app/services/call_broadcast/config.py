...
class Settings(BaseSettings):
    ...
    # 用户第 N 条消息内必须收到一次 App 链接；1 表示第一条用户消息后的回复就要带链接。
    APP_DOWNLOAD_EARLY_LINK_USER_MESSAGE_LIMIT: int = 1
    ...
    # 独立固定自动回复模块：只发送运营配置的文字和媒体，不进入 AI/话术/培育链路。
    FIXED_AUTO_REPLY_ENABLED: bool = True
    # Legacy combined tick interval (enqueue job uses ENQUEUE_POLL_SECONDS).
    FIXED_AUTO_REPLY_POLL_SECONDS: int = 20
    FIXED_AUTO_REPLY_ENQUEUE_POLL_SECONDS: int = 60
    FIXED_AUTO_REPLY_SEND_POLL_SECONDS: int = 5
    FIXED_AUTO_REPLY_RECOVERY_POLL_SECONDS: int = 60
    FIXED_AUTO_REPLY_SCHEDULER_MAX_INSTANCES: int = 1
    FIXED_AUTO_REPLY_PER_ACCOUNT_BATCH: int = 10
    FIXED_AUTO_REPLY_MAX_TOTAL_BATCH: int = 200
    FIXED_AUTO_REPLY_MAX_PARALLEL_ACCOUNTS: int = 16
    FIXED_AUTO_REPLY_SENDING_TIMEOUT_SECONDS: int = 300
    FIXED_AUTO_REPLY_MAX_SEND_ATTEMPTS: int = 3
    FIXED_AUTO_REPLY_TEXT_SEND_TIMEOUT_SECONDS: int = 15
    FIXED_AUTO_REPLY_MEDIA_SEND_TIMEOUT_SECONDS: int = 60
    FIXED_AUTO_REPLY_ASSET_FAIL_CIRCUIT: int = 5
    FIXED_AUTO_REPLY_SEND_TICK_TIMEOUT_SECONDS: int = 90
    ...
    # LLM 记忆评分返回非 JSON 时，用规则保存明确的年龄/国家/偏好/日程等事实。
    MEMORY_WRITE_FALLBACK_ENABLED: bool = True
    ...
    POST_INBOUND_VIDEO_RELEASE_REVIEW_CALLS: int = 6
    ...
    CALL_BROADCAST_ANSWER_TIMEOUT_SECONDS: int = 60
    CALL_BROADCAST_PLAYBACK_TIMEOUT_BUFFER_SECONDS: int = 90
    CALL_BROADCAST_DIAL_TEARDOWN_BUFFER_SECONDS: int = 45
    CALL_BROADCAST_PREP_TIMEOUT_SECONDS: int = 180
    CALL_BROADCAST_STALE_ACTIVE_BUFFER_SECONDS: int = 120
    CALL_BROADCAST_STALE_ACTIVE_MIN_SECONDS: int = 300
    # Optional ceiling for inbound auto-answer dial+stream wall clock (0 = derive from media).
    CALL_BROADCAST_INCOMING_WALL_TIMEOUT_SECONDS: int = 0
