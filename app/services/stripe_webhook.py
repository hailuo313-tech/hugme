...
async def _mark_result(
    ...
    *,
    ...
    try:
        ...
        logger.bind(component="stripe_webhook", event_id=event_id).exception("stripe_webhook.mark_result_failed")
...
async def handle_event(db: AsyncSession, event: dict[str, Any]) -> str:
    ...
    log = logger.bind(component="stripe_webhook", event_id=event_id, event_type=event_type)
    try:
...
async def _handle_checkout_completed(db: AsyncSession, event: dict[str, Any]) -> None:
    ...
    if user_id is not None:
        await db.execute(
            text("UPDATE users SET vip_level = COALESCE(vip_level, 0) + 1 WHERE id = :uid"),
            {"uid": user_id},
        )
    await db.commit()
