...
def start_scheduler() -> Optional[AsyncIOScheduler]:
    ...
    if not settings.SILENT_REACTIVATION_ENABLED:
        logger.bind(component="silent_reactivation").info(
            "silent_reactivation.scheduler.disabled"
        )
        return None
...
def shutdown_scheduler() -> None:
    ...
    try:
        ...
        logger.bind(component="silent_reactivation").exception(
            "silent_reactivation.scheduler.shutdown_error"
        )
    finally:
        _scheduler = None
        logger.bind(component="silent_reactivation").info(
            "silent_reactivation.scheduler.stopped"
        )
