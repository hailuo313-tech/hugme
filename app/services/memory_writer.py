...
async def maybe_write_memory(
    ...
    if redis is not None:
        try:
            ...
            log.bind(error_type=type(exc).__name__).warning("memory.write.dedup.redis_failed")
            is_dup = False
        if is_dup:
    ...
    if not evaluation.get("is_memory_worthy"):
        log.bind(memory_type=evaluation.get("memory_type")).info("memory.write.below_threshold")
        return None
    ...
    if score < threshold:
        log.bind(score=score, threshold=threshold).info("memory.write.below_threshold")
        return None
...
async def _persist_memory(
    ...
    await db.execute(
        ...
        ),
        {
            ...
            "tags": json.dumps(evaluation.get("emotion_tags", []), ensure_ascii=False),
