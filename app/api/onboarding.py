...
async def _get_or_create_user(db: AsyncSession, channel: str, external_id: str) -> str:
    row = (
        await db.execute(
            text("SELECT id FROM users WHERE channel=:ch AND external_id=:eid"), {"ch": channel, "eid": external_id}
        )
    ).fetchone()
    if row:
        return str(row[0])
...
async def _get_profile_prefs(db: AsyncSession, user_id: str) -> dict:
    row = (
        await db.execute(text("SELECT preferences FROM user_profiles WHERE user_id=:uid"), {"uid": user_id})
    ).fetchone()
...
async def _ensure_aria_exists(db: AsyncSession):
    ...
    row = (await db.execute(text("SELECT id FROM characters WHERE id=:cid"), {"cid": DEFAULT_CHARACTER_ID})).fetchone()
    if not row:
        await db.execute(
            text("""
...
async def submit_onboarding_step(
    ...
    if current_step >= ONBOARDING_STEPS + 1:
        ...
        char_row = (
            await db.execute(
                text("SELECT current_character_id FROM user_profiles WHERE user_id=:uid"), {"uid": user_id}
            )
        ).fetchone()
