...
async def ai_reply(
    ...
    try:
        ...
        log.bind(result="failed", reason=str(exc)).warning("conversation.reply.orchestrator_failed")
        raise HTTPException(
            status_code=503,
    ...
    await db.execute(
        text(
            ...
            "VALUES (:id,:cid,'assistant','bot',:ct,'text')"
        ),
        {"id": bot_msg_id, "cid": conv_id, "ct": reply_text},
    )
    await db.execute(
    ...
    try:
        ...
        log.bind(error_type=type(exc).__name__).warning("conversation.reply.ctx_push_failed")
