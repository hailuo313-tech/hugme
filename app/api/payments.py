...
async def create_order(
    ...
    if not stripe_key:
        log.bind(result="failed", reason="STRIPE_SECRET_KEY not set").error("payments.stripe.config_missing")
    ...
    checkout_url_raw = session.url
    if not checkout_url_raw:
        raise HTTPException(status_code=502, detail="Stripe session missing checkout URL")
    checkout_url: str = checkout_url_raw
    log.bind(order_id=order_id, session_id=session_id).info("payments.stripe.session.created")

    # ── 3. 更新 orders.provider_order_id ────────────────
    await db.execute(
        text("UPDATE orders SET provider_order_id = :sid WHERE id = :oid"),
...
async def stripe_webhook(
    ...
    try:
        ...
        log.bind(result="signature_failed", reason=str(exc)).warning("stripe_webhook.verify_failed")
