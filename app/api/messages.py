...
async def inbound_message(
    ...
    idem_key = idempotency_key or (f"tg-{meta['tg_message_id']}" if meta.get("tg_message_id") else None)
    ...
    if row:
        ...
        await db.execute(text("INSERT INTO user_profiles (user_id) VALUES (:uid)"), {"uid": user_id})
        await db.commit()
    ...
    await db.execute(
        ...
        {"id": msg_id, "cid": conv_id, "sid": user_id, "ct": data.content, "ctype": data.message_type},
    )
    await db.execute(
        text("UPDATE conversations SET last_message_at=NOW(), updated_at=NOW() WHERE id=:id"), {"id": conv_id}
    )
    await db.commit()
