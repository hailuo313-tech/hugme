...
async def create_memory(user_id: str, data: MemoryCreate, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(
        text(
            "INSERT INTO memories (id,user_id,memory_type,content,importance_score,memory_scope,character_id) VALUES (:id,:uid,:mt,:ct,:sc,:ms,:cid)"
        ),
        {
            "id": mid,
            "uid": user_id,
            "mt": data.memory_type,
            "ct": data.content,
            "sc": data.importance_score,
            "ms": data.memory_scope,
