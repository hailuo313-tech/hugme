    ...
    <AdminFrame operator={operator} active="ai" title={title} subtitle={subtitle}>
      <section className="rounded-md border border-slate-800 bg-slate-900 p-5">
        ...
        <p className="mt-4 text-sm text-slate-500">
          当前子页面为独立入口，后续你确认分工后我可按原有 `/admin/ai-ops` 功能区块继续拆分迁移到这里。
        </p>
      </section>
      ...
      <div className="mt-4">
        <a
          href="/admin/ai-ops"
          className="inline-flex rounded-md border border-slate-700 px-4 py-2 text-sm text-slate-200 transition hover:bg-slate-800"
        >
          返回 AI 话术与人设主页
        </a>
      </div>
...
export default function ProtectedSubPage({
  title,
  subtitle,
  description,
  children,
}: Omit<SubPageFrameProps, "operator">) {
  return (
    <AuthGate>
      {(operator) => (
        <SubPageFrame
