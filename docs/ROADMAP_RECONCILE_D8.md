...
| D8-4 | Second beta + data dashboard: D1 retention / score distribution / token cost; seven-day first batch data | First-day metric pack is merged in PR [#34](https://github.com/hailuo313-tech/hugme/pull/34): [docs/BETA_DAY1_METRICS.md](BETA_DAY1_METRICS.md) and [scripts/beta/day1_metrics.sh](../scripts/beta/day1_metrics.sh). PR [#29](https://github.com/hailuo313-tech/hugme/pull/29) is merged and adds [docs/D8_4_BETA_DASHBOARD.md](D8_4_BETA_DASHBOARD.md), [docs/D8_ROUND2_METRICS_SPEC.md](D8_ROUND2_METRICS_SPEC.md), [scripts/beta/d8_4_report.sh](../scripts/beta/d8_4_report.sh), and the RUNBOOK D8-4 section. [docs/D8_4_BETA_RUN_20260515.md](D8_4_BETA_RUN_20260515.md) captures a read-only production run for the currently reachable 7-day window: 9 users, 3 onboarding completed, 5 users with assistant replies, D1 retention 11.1%, and score distribution. | D8-4a Day-1 metrics and D8-4b report package are on `main`. D8-4c has first reachable 7-day-window evidence, but the cohort has not aged through a full seven calendar days. D8-4d token cost is not connected to provider usage yet; the report only has persisted-message token floor estimates. | Keep D8-4 partial unless the roadmap narrows the card to "first reachable beta run." Remaining follow-up: persist provider token usage / billing source, then rerun after the first cohort reaches a full seven days. |
