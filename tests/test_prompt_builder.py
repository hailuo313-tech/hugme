...
def test_l4_renders_known_stages(stage, marker):
    out = build_prompt(PromptInput(user_text="hi", profile={"relationship_stage": stage}))
...
def test_l4_vip_level_shown_when_paid():
    out = build_prompt(PromptInput(user_text="hi", profile={"relationship_stage": "S2", "vip_level": 2}))
    body = out.layers["L4_RELATIONSHIP"]
...
def test_l5_handles_forbidden_topics_as_string():
    out = build_prompt(PromptInput(user_text="hi", profile={"forbidden_topics": "政治"}))
    body = out.layers["L5_USER_PROFILE"]
    assert "政治" in body
...
def test_l7_loneliness_band(score, band_keyword):
    out = build_prompt(PromptInput(user_text="hi", profile={"loneliness_score": score}))
    body = out.layers["L7_CONVERSATION_STATE"]
    assert band_keyword in body


def test_l7_missing_score_falls_back_to_cold_start():
    out = build_prompt(PromptInput(user_text="hi", profile={"loneliness_score": None}))
    body = out.layers["L7_CONVERSATION_STATE"]
    assert "冷启动" in body or "默认" in body


def test_l7_invalid_score_does_not_crash():
    out = build_prompt(PromptInput(user_text="hi", profile={"loneliness_score": "n/a"}))
    body = out.layers["L7_CONVERSATION_STATE"]
    assert "冷启动" in body or "默认" in body
