...
def _result(rows: list[Any] | None = None, scalar: Any = None):
    ...
    res.fetchone.return_value = rows[0] if rows else ((scalar,) if scalar is not None else None)
    return res
...
def test_detail_happy_path_returns_messages():
    ...
    msg2 = _row(
        {**msg1._mapping, "id": "m2", "sender_type": "assistant", "content": "嗨～", "model_name": "openrouter/x"}
    )
